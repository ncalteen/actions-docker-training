...
def create_label(repo_id, label):
  ...
  with open(path.join(path.dirname(__file__), "queries/create_label.gql"),
            "r") as query_file:
...
def get_labels(owner, repo):
  ...
  with open(
      path.join(path.dirname(__file__), "queries/get_repo_data.gql"),
      "r") as query_file:
    query = "".join(query_file.readlines())
  ...
  if status_code >= 200 and status_code <= 300:
    ...
    raise Exception(
        "[ERROR] getting issue labels. Status Code: {status_code} - Message: {result}"
        .format(status_code=status_code, result=result["message"]))
...
def delete_label(label_id):
  ...
  with open(path.join(path.dirname(__file__), "queries/delete_label.gql"),
            "r") as query_file:
    query = "".join(query_file.readlines())

  payload = {"query": query, "variables": query_variables}
  result = requests.post(
      api_url, data=json.dumps(payload), headers=headers).json()
...
def copy_labels(source_repo, target_repo, dry):
  ...
  try:
    print("Fetching labels for {source_repo_name} repo.".format(
        source_repo_name=source_repo_name))
    _, source_repo_labels = get_labels(source_owner, source_repo_name)
    print("Fetched labels for {source_repo_name}".format(
        source_repo_name=source_repo_name))

    print("Fetching labels for {target_repo_name} repo.".format(
        target_repo_name=target_repo_name))
    target_repo_id, target_repo_labels = get_labels(target_owner,
                                                    target_repo_name)
    print("Fetched labels for {target_repo_name}".format(
        target_repo_name=target_repo_name))
    ...
    if dry:
      ...
      print("{label_count} labels would have been created.".format(
